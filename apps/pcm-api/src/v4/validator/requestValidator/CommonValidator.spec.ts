...
describe('CommonValidator', () => {
  ...
  describe('validateEmail', () => {
    it(`should pass validation when email is valid`, (done) => {
      const email = 'xyz@email.com';
      try {
        const errors = commonValidator.validateEmail(email);
        expect(errors.length, JSON.stringify(errors)).to.equal(0);
        done();
      } catch (error) {
        done(error);
    ...
    it(`should return error when email is invalid `, (done) => {
      ...
      const errors = commonValidator.validateEmail(email);
      expect(errors.length, JSON.stringify(errors)).to.equal(1);
      expect(errors[0]).to.equal(`Invalid email ${email}`);
      done();
    });
  });
  ...
  describe('validateOffsetCursor', () => {
    ...
    it(`should pass validation when offsetCursor for parts is valid`, (done) => {
      const offsetCursor =
        '6d4fb6ac-55f9-4a7a-9cfb-81085a8a6896:6d4fb6ac-55f9-4a7a-9cfb-81085a8a6898:0_6d4fb6ac-55f9-4a7a-9cfb-81085a8a6898_desc';
      const errors = commonValidator.validateOffsetCursor(offsetCursor, true);
      expect(errors.length, JSON.stringify(errors)).to.equal(0);
      done();
    });
    ...
    it(`should return error when value of offsetCursor string has invalid UUID`, (done) => {
      const offsetCursor =
        'x:6d4fb6ac-55f9-4a7a-9cfb-81085a8a6898:6d4fb6ac-55f9-4a7a-9cfb-81085a8a6898_asc';
      const errors = commonValidator.validateOffsetCursor(offsetCursor);
      expect(errors.length, JSON.stringify(errors)).to.equal(1);
      expect(errors[0]).to.equal(`Invalid offsetCursor : ${offsetCursor}`);
      done();
