...
class CommonValidator {
  ...
  private getUUIDFromOffsetCursor(offsetCursor: string): string[] {
    const uuids: string[] = offsetCursor.split(':').map((uuid) => {
      if (uuid.includes('_asc')) return uuid.replace('_asc', '');
      else if (uuid.includes('_desc')) return uuid.replace('_desc', '');
      else return uuid;
    });
    return uuids;
  }

  private getUUIDFromOffsetCursorForParts(offsetCursor: string): string[] {
    const uuids: string[] = offsetCursor.split(':').map((uuid) => {
